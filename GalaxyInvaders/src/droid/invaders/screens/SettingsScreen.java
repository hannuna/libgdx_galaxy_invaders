...
public class SettingsScreen extends Screens {
	...
	public SettingsScreen(final DroidInvadersMain game) {
		super(game);
		...
		aceletometerSlider.addListener(new ChangeListener() {
			...
			public void changed(ChangeEvent event, Actor actor) {
				int value = 21 - (int) ((Slider) actor).getValue();
				Settings.aceletometerSensitive = value;
		...
		menuControls.add(new Label(idiomas.getTraduccion("on_screen_control"), Assets.styleLabel)).left();
		menuControls.add(onScreenControl).size(25);
		menuControls.row().padTop(10);
		menuControls.add(new Label(idiomas.getTraduccion("tilt_control"), Assets.styleLabel)).left();
		...
		oNave = new Nave(WORLD_SCREEN_WIDTH / 2, WORLD_SCREEN_HEIGHT / 3); // Coloco la nave en posicion
		this.camRender = new OrthographicCamera(WORLD_SCREEN_WIDTH, WORLD_SCREEN_HEIGHT);
		camRender.position.set(WORLD_SCREEN_WIDTH / 2, WORLD_SCREEN_HEIGHT / 2, 0);
		// menuControls.debug();
	...
	public void draw(float delta) {
		...
		batcher.begin();
		Assets.font45.draw(batcher, idiomas.getTraduccion("control_options"), 10, 460);

		if (Settings.isTiltControl) {
			String tiltSensitive = idiomas.getTraduccion("tilt_sensitive");
			bounds = Assets.font15.getBounds(tiltSensitive);
			Assets.font15.draw(batcher, tiltSensitive, SCREEN_WIDTH / 2f - bounds.width / 2f, 335);
			batcher.draw(Assets.clickAyuda, 155, 0, 10, 125);
		}
		else {
			String speed = idiomas.getTraduccion("speed");
			bounds = Assets.font15.getBounds(speed);
			Assets.font15.draw(batcher, speed, SCREEN_WIDTH / 2f - bounds.width / 2f, 335);
